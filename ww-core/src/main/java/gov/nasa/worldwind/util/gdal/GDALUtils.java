...
public class GDALUtils
{
    ...
    static {
        try
        {
            ...
            boolean gdalJNIAvailable = gdalJNI.isAvailable();
            boolean gdalConstJNIAvailable = gdalconstJNI.isAvailable();

            if (gdalJNIAvailable && gdalConstJNIAvailable)
            {
            ...
            {
                if(!gdalJNIAvailable){
                    Logging.logger().warning("Gdal JNI not available.");
                }
                if(!gdalConstJNIAvailable){
                    Logging.logger().warning("GDAL const JNI not available.");
                }
            }
        }
        catch (Throwable t)
    ...
    public static boolean isGDALAvailable(){
        return gdalIsAvailable.get();
    }

    protected static String findGdalDataFolder()
    {
        return GDALDataFolderUtil.getGDALDataPath().getAbsolutePath();
    }
